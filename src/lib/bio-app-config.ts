...
export type BioQueueConfigKey = (typeof BIO_QUEUE_KEYS)[number];
...
export function getBioQueueConfigNumber(map: Map<string, string>, key: string, defaultNum: number): number {
...
export function getMaxConcurrentFromMap(map: Map<string, string>, queueName: string): number {
  const key = `bio_queue_max_${queueName}` as BioQueueConfigKey;
  const defaults: Record<string, number> = { fila_1x: 6, fila_20x: 4, fila_100x: 3, fila_1000x: 2 };
  return getBioQueueConfigNumber(map, key, defaults[queueName] ?? 3);
}

export function getUseQueueFromMap(map: Map<string, string>, queueName: string): boolean {
  const key = `bio_queue_use_queue_${queueName}` as BioQueueConfigKey;
  const raw = map.get(key) ?? DEFAULTS[key] ?? "";
