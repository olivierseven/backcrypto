...
export type BinanceKlineCacheRefreshResult = {
  ok: true;
  totalRows: number;
...
export type RunBinanceKlineCacheRefreshOptions = {
  onSymbolDone?: (symbol: string) => void;
};

/** Mesma lógica do Debug → Refresh cache (target=prod). */
export async function runBinanceKlineCacheRefresh(
  db: PrismaClient,
  options: RunBinanceKlineCacheRefreshOptions = {}
): Promise<BinanceKlineCacheRefreshResult> {
