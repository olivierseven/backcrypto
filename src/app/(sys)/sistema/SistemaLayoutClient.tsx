...
function SistemaHeader({
  ...
  const isMercadosPage = pathname === "/mercados" || pathname?.endsWith("/mercados") === true;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Link
          href="/mercados"
          className={`p-1.5 sm:p-2 rounded shrink-0 ${isMercadosPage ? "bg-zinc-200 text-zinc-900" : "text-zinc-700 hover:bg-zinc-100"}`}
          aria-label={(t as Record<string, string>).menuMercados ?? "Markets"}
          title={(t as Record<string, string>).menuMercados ?? "Markets"}
          aria-current={isMercadosPage ? "page" : undefined}
        >
          <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
            <path
              strokeLinecap="round"
...
function SistemaLayoutContent({
  ...
  return (
    <>
            ...
            {isSistemaChartPage && <MobileAsciiKeyboard />}
