...
/**
 * POST: `GET /api/v3/order` na Binance e atualiza o registo em `UserBinanceSpotOrder`
 * (ex.: após compra LIMIT NEW quando o spot cruza o limite — refresh único no cliente).
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const res = await binanceSignedGet("/api/v3/order", creds.apiKey, creds.apiSecret, {
      symbol: sym,
      orderId,
      recvWindow: "5000",
    });
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "binance_error", code: j?.code, msg: j?.msg ?? "query_failed" },
        { status: 400 }
      );
    }

    const orderJson = res.json;
    if (typeof orderJson !== "object" || orderJson === null) {
      return NextResponse.json({ error: "invalid_binance_response" }, { status: 502 });
    }
    ...
    await cryptoPrisma.userBinanceSpotOrder.update({
      ...
      data: {
        ...
        ...(isCanceled && row.canceledAt == null ? { canceledAt: new Date() } : {}),
      },
    });

    return NextResponse.json({ success: true, order: orderJson });
  } catch (e) {
    console.error("[binance-connection/order/sync POST]", e);
