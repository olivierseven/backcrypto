...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await cryptoPrisma.userNotification.updateMany({
      where: { idNotification: notificationId, userId },
      data: { ativo: false, updatedAt: new Date() },
    });

    if (result.count === 0) return NextResponse.json({ error: "Notificação não encontrada" }, { status: 404 });
    return NextResponse.json({ success: true, message: "Notificação excluída com sucesso" });
  } catch (error) {
    console.error("[backcrypto/notifications/delete]", error);
