 ...
 * Body: { symbol?: string, interval: "1m" | "1h", from: number, to: number } (from/to em ms UTC)
 * Ou: { symbol?: string, gaps: { interval: "1m"|"1h", from: number, to: number }[] }
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    type Gap = { interval: "1m" | "1h"; from: number; to: number };
    let gaps: Gap[];
    if (Array.isArray(body.gaps) && body.gaps.length > 0) {
      gaps = body.gaps.filter((g: unknown) => {
        const x = g as Gap;
        return g != null && typeof g === "object" && (x.interval === "1m" || x.interval === "1h") && typeof x.from === "number" && typeof x.to === "number";
      }) as Gap[];
    } else if (
      (body.interval === "1m" || body.interval === "1h") &&
      ...
      return NextResponse.json(
        { error: "Body: { interval: '1m'|'1h', from: number, to: number } ou { gaps: [...] }" },
        { status: 400 }
      );
    }

    const stepMs = (interval: "1m" | "1h") => (interval === "1m" ? ONE_MINUTE_MS : ONE_HOUR_MS);
    let totalInserted1m = 0;
    let totalInserted1h = 0;
    const details: { interval: string; from: number; to: number; fetched: number; inserted: number }[] = [];
    ...
    for (const gap of gaps) {
      ...
      while (cursor <= endTimeInclusive) {
        ...
        if (klines.length === 0 && cursor === startTime && gap.interval === "1h") {
