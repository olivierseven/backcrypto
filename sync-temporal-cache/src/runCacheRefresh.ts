/**
 * Mesma lógica que src/lib/binance-kline-cache-refresh.ts (Debug → Refresh cache prod).
 * Manter os dois ficheiros alinhados.
 */
import { Prisma, type PrismaClient } from "@prisma/client";
import { getKlineSymbolsFromDb, getKlineSymbolsWithPeriods } from "./klineSymbols.js";
...
export type CacheRefreshResult = {
  ok: true;
  totalRows: number;
  details: { symbol: string; interval: string; rows: number }[];
  purgedFast: number;
  purged5m: number;
  purged1h: number;
};
...
export type RunCacheRefreshOptions = {
  onSymbolDone?: (symbol: string) => void;
};

export async function runCacheRefresh(
  db: PrismaClient,
  options: RunCacheRefreshOptions = {}
